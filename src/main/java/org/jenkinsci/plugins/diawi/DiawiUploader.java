...
public class DiawiUploader
  ...
  public void perform(
    ...
    for (String file : fileName.split(",")) {
      try {
        file = file.trim();
        DiawiRequest dr = new DiawiRequest(
          token,
          proxyHost,
          proxyPort,
          proxyProtocol
        ...
        DiawiRequest.DiawiJobStatus S = job.getStatus(
          token,
          proxyHost,
          proxyPort,
          proxyProtocol
        ...
        listener
          .getLogger()
          .println(path + " have been uploaded successfully to diawi ... ");

        if (S.status == 2001) throw new Exception(
          "Looks like upload job hanged. please login to Diawi.com and check the uplaod status"
        ); else if (S.status == 4000) throw new Exception(
          "Upload Failed, looks like you chose the wrong file"
        ); else if (S.status != 2000) throw new Exception(
